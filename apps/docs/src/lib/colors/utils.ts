...
export const generateRadixColors = ({
	...
	let primaryScaleColors = getScaleFromColor(primaryBaseColor, allScales, backgroundColor)
	...
	primaryScaleColors[8] = primary9Color
	primaryScaleColors[9] = getButtonHoverColor(primary9Color, [primaryScaleColors])

	// Limit saturation of the text colors
	primaryScaleColors[10].coords[1] = Math.min(
		Math.max(primaryScaleColors[8].coords[1], primaryScaleColors[7].coords[1]),
		primaryScaleColors[10].coords[1],
	)
	primaryScaleColors[11].coords[1] = Math.min(
		Math.max(primaryScaleColors[8].coords[1], primaryScaleColors[7].coords[1]),
		primaryScaleColors[11].coords[1],
	)

	const primaryScaleHex = primaryScaleColors.map((color) =>
		color.to("hsl").toString({ format: "hsl" }),
	) as ArrayOf12<string>

	const primaryScaleWideGamut = primaryScaleColors.map(toOklchString) as ArrayOf12<string>
...
export const getFullColorScales = (color: string, backgroundColor: Color, scales: Record<string, ArrayOf12<Color>>) => {
...
function getAlphaColor(
	...
	const clampRgb = (n: number) => (Number.isNaN(n) ? 0 : Math.min(rgbPrecision, Math.max(0, n)))
	const clampA = (n: number) => (Number.isNaN(n) ? 0 : Math.min(alphaPrecision, Math.max(0, n)))
