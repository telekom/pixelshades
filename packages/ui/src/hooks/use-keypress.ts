...
function checkCombination(event: KeyboardEvent, keys: Array<keyof typeof Keys>): boolean {
...
export function useKeyPress({
	...
	useEffect(() => {
		const keydownListener = (event: KeyboardEvent) => {
			for (const { keys, event: triggerEvent, preventDefault = true } of keyPressItems) {
